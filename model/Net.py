...
class Net(nn.Module):
    def __init__(self, encoder, decoder, start_iter):
        ...
        if (start_iter > 0):
            self.transform.load_state_dict(torch.load('weight/transformer_iter_' + str(start_iter) + '.pth'), strict = False)
            self.decoder.load_state_dict(torch.load('weight/decoder_iter_' + str(start_iter) + '.pth'), strict = False)
